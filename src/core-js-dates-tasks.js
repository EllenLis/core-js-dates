...
function getNextFriday(date) {
  ...
  const daysUntilNextFriday = (5 + 7 - dayOfWeek) % 7 || 7;
...
function getWeekNumberByDate(date) {
  ...
  const dayOfWeekJan1 = yearStart.getDay() || 7;
  if (dayOfWeekJan1 !== 1) {
    yearStart.setDate(yearStart.getDate() - dayOfWeekJan1 + 1);
  }
  const diffInDays = Math.floor((date - yearStart) / (24 * 60 * 60 * 1000));
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    if (dayCount % cycleLength < countWorkDays) {
      schedule.push(formatDateToDMY(currentDate));
    }
